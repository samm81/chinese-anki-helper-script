...
class FlashcardHelper(cmd.Cmd):
    ...
    def handle_word(self, word_to_flashcard=word_to_ChineseRecognitionOnly):
        if len(self.word.english) is 1:
            flashcard = word_to_flashcard(self.word, self.tags)
    ...
    def handle_english(self):
        primaries = [english for i, english in enumerate(
            self.word.english) if i in self.choice]
        secondaries = [english for i, english in enumerate(
            self.word.english) if i not in self.choice]
        definition = '; '.join(primaries)
        extra_definition = '; '.join(secondaries)
        tags = self.tags
        flashcard = ChineseRecognitionOnly(self.word.simplified, self.word.traditional,
                                           self.word.pinyin, self.word.zhuyin, definition, extra_definition, '', '', tags)
        return self.save_to_file(flashcard)

    def display_options(self, optionstrings, multiple=False):
        print('\n'.join('{}) {}'.format(i, optionstring)
                        for i, optionstring in enumerate(optionstrings)))
        if multiple:
            print(
                'type `pick [num] ([num] [num]) to pick option(s), or type any other command')
        else:
            print(
                'type `pick [num]` to pick an option, or type any other command')

    def save_to_file(self, flashcard, filename=filename):
        confirm = input(
            f'saving {flashcard} to file {filename}, looks ok ([Y]n) ? ')
    ...
    def do_find(self, arg):
        ...
        if len(words) is 1 and self.filemode:
            ...
            self.display_options('{}\t{}\t{}\t{}\t{}'.format(*word)
                                 for word in words)
    ...
    def do_make(self, arg):
        ...
        self.save_to_file(ChineseRecognitionOnly(
            simplified, traditional, pinyin, zhuyin, english, '', '', '', self.tags))
        self.transition_to(State.BEGIN)

    def do_tags(self, arg):
        'set tags: tags IntegratedChinese::1.4 IntermediateReader::8'
        self.tags = arg
    ...
    def register_filemode(self):
        """
        used when reading a set of words from a file so that the cmd knows to
        finish after saving a card
        """
        self.filemode = True
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Command line helper for making Chinese Anki flashcards')
    parser.add_argument('file', nargs='?', type=argparse.FileType(
        'r', encoding='utf-8'), help='newline separated file of words to add')
